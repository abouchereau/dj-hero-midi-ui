    ...
    scanOutput(index, callback) {
        ...
        setInterval(()=> {//process.onNextTick ?
            if (this.midiOutput.getPortCount() != this.nbMidiDevices) {
                ...
                if ((this.nbMidiDevices > 0 && this.midiOutIndex == -1) || (this.midiOutIndex > this.nbMidiDevices.length-1)) {
                    this.openFromIndex(index<=this.nbMidiDevices?index:0);//default
                }
                this.devices = [];
